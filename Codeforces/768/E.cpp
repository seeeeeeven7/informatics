...
// Brute-force sg calculation
int calc(int n) {
	vector<int> nsg;
	for (int i = 1; i <= n; i++) if (!bo[i]) {
		bo[i] = true;
		nsg.push_back(calc(n - i));
		bo[i] = false;
	}
	sort(nsg.begin(), nsg.end());
	//cout << n << ':' << endl;
	//for (int i = 0; i < nsg.size(); i++) cout << nsg[i] << ' '; cout << endl;
	int sg = 0;
	for (int i = 0; i < nsg.size(); i++) if (nsg[i] == sg) sg++;
	return sg;
}
...
//	bo = vector<bool>(61, false);
//	for (int i = 1; i <= 50; i++) cout << "! " << i << ' ' << calc(i) << endl;

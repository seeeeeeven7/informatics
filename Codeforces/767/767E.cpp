...
struct node
{
    ...
    long long priority;
    int value;
    node(long long  priority_, int value_):priority(priority_), value(value_) {}
}; 
...
int main() {
	...
	while (cin >> n >> m) {
		...
		long long ans = 0;
		for (int i = 0; i < n; i++) {
			if (a[i] % 100 > 0) heap.push(node(b[i] * (100 - a[i] % 100), i));
			//cout << i << ' ' << m << ' ' << a[i] % 100 << endl;
			while (m < a[i] % 100) {
				//cout << heap.top().priority << ' ' << heap.top().value << endl;
				ans += heap.top().priority;
				c[heap.top().value] = true;
				m += 100;
				heap.pop();
			}
			m -= a[i] % 100;
		...
		cout << ans << endl;
		for (int i = 0; i < n; i++) {
			if (!c[i]) cout << a[i] / 100 << ' ' << a[i] % 100 << endl;
			else cout << a[i] / 100 + 1 << ' ' << 0 << endl;
		}
	}

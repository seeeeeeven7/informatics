...
int32_t main() {
	...
	while (cin >> n) {
		...
		vector<vector<int>> c(6, vector<int>(46, 0));
		for (string s : a) {
			// cout << s.length() << ' ' << dsum(s) << endl;
			c[s.length()][dsum(s)] ++;
		}
		int ans = 0;
		for (string s : a) {
			...
			for (int i = 1; i < m; i++) 
				if ((m + i) % 2 == 0) {
					int j = (m + i) / 2;
					// cout << s.substr(0, j) << ' ' << s.substr(j, m - j) << endl;
					// cout << s.substr(m - j, j) << ' ' << s.substr(0, m - j) << endl;
					int l = dsum(s.substr(0, j)) - dsum(s.substr(j, m - j));
					int r = dsum(s.substr(m - j, j)) - dsum(s.substr(0, m - j));
					if (l >= 0)
						ans = ans + c[i][l];
					if (r >= 0)
						ans = ans + c[i][r];

...
void find(vector<int> &f, vector<int> &g, int x) {
	if (f[x] == x) return;
	find(f, g, f[x]);
	g[x] ^= g[f[x]];
	f[x] = f[f[x]];
...
bool merge(vector<int> &f, vector<int> &g, int x, int y, int k) {
	cout << x << ' ' << y << ' ' << k << endl;
	find(f, g, x); find(f, g, y);
	print1d(f);
	print1d(g);
	if (f[x] != f[y]) {
		// cout << f[x] << ' ' << f[y] << endl;
...
int main() {
	...
	while (tasks --) {
		...
		while (k --) {
			...
			a.pb(tri(x1, min(y1, y2), y1 < y2));
		}
		sort(a.begin(), a.end(), cmp);
		// for (auto p : a) cout << p.x << ' ' << p.y << ' ' << p.d << endl;
		bool ans = true;
		vector<int> f(m), g(m);
		for (int i = 1; i < m; i++) f[i] = i;
		for (int i = 1; i < m; i++) g[i] = 0;

		for (int i = 0, j = 0; i < (int)a.size() && ans; i++)
			if (i == (int)a.size() - 1 || a[i].x != a[i + 1].x) {
				for (int k = j; k < i; k++) {
					ans = ans & merge(f, g, a[i].y, a[j].y, a[i].d ^ a[j].d);
				}
				j = i + 1;
			}

...
void calc(vector<int> s, int n, int r, int l) {
	if (r == 0) {
		c[s] = a.size();
		a.push_back(s);
		b.push_back(n);
...
int dfs(int i, int f) {
	...
	if (size <= MAXM) {
		...
		idxs[i] = c[subtree_idxs];
//		for (auto j : subtree_idxs) cout << j << ' '; cout << endl;
//		cout << i << ' ' << idxs[i] <<endl;
		found[idxs[i]] = true;
	}
	return size;
...
void resolve(vector<int> s, int i) {
	int k = i + 1;
	for (auto j : s) {
		cout << i << ' ' << k << endl;
		resolve(a[j], k);
...
int main() {
	...
	vector<int> subtrees;
	a.push_back(subtrees);
	b.push_back(1);
	c[subtrees] = 0;
	for (int i = 2; i <= MAXM; i++) {
		calc(subtrees, i, i - 1, 0);
	}
...
//		cout << s << endl;
		if (s > n) {
			res(1, 0);
       		...
       		resolve(base, n - s + 1);
			for (int i = 1; i <= n - s; i++) cout << i << ' ' << i + 1 << endl;
		}
	}

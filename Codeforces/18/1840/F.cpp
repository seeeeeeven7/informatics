...
int expand(vector<vector<int>> &f, int l, int r) {
	// cout << l << ' ' << r << endl;
	queue<pii> q;
	for (int i = 0; i <= n; i++)
		for (int j = 0; j <= m; j++)
...
int32_t main() {
	...
	while (tasks --) {
		...
		int r; cin >> r;
		vector<int> t(r), d(r), co(r), ti(r);
		for (int i = 0; i < r; i++) 
			cin >> t[i] >> d[i] >> co[i];
		for (int i = 0; i < r; i++)
			ti[i] = i;
		sort(ti.begin(), ti.end(), [&t](int i, int j) { return t[i] < t[j]; });
		// print1d(ti);
		vector<vector<int>> f(n + 1, vector<int>(m + 1, -1)); f[0][0] = 0;
		int ans = -1;
		for (int i = 0, j = -1; i < r; i++) {
			if (i == r - 1 || t[ti[i]] != t[ti[i + 1]]) {
				int cur = expand(f, j < 0 ? 0 : t[ti[j]], t[ti[i]]);
				if (cur != -1 && (ans == -1 || ans > cur)) ans = cur;
				for (int k = j + 1; k <= i; k++) {
					// cout << "L " << d[ti[k]] << ' ' << co[ti[k]] << endl;
					if (d[ti[k]] == 1) {
						for (int y = 0; y <= m; y++) f[co[ti[k]]][y] = -1;
					}
					else {
						for (int x = 0; x <= n; x++) f[x][co[ti[k]]] = -1;
					}
				}
				j = i;
			}
		}
		int cur = expand(f, t[ti[r - 1]], -1);
		if (cur != -1 && (ans == -1 || ans > cur)) ans = cur;
		cout << ans << endl;
	}

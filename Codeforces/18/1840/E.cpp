...
int32_t main() {
	...
	while (tasks --) {
		...
		for (int i = 0; i < (int)a.length(); i++)
			c[0] += (a[i] != b[i]);
		int t, q; cin >> t >> q;
		vector<bool> bl(a.length(), false);
		queue<pii> fu;
		while (q --) {
			int op; cin >> op;
			while (fu.size() > 0 && fu.front().fi == q) {
				int p = fu.front().se; fu.pop();
				c[1] -= (a[p] != b[p]);
				c[0] += (a[p] != b[p]);
				bl[p] = false;
			}
			...
			if (op == 2) {
				int x1, y1, x2, y2; cin >> x1 >> y1 >> x2 >> y2; y1--; y2--;
				c[bl[y1]] -= (a[y1] != b[y1]);
				if (y1 != y2) c[bl[y2]] -= (a[y2] != b[y2]);
				if (x1 == 1 && x2 == 1) swap(a[y1], a[y2]);
				if (x1 == 1 && x2 == 2) swap(a[y1], b[y2]);
				if (x1 == 2 && x2 == 1) swap(b[y1], a[y2]);
				if (x1 == 2 && x2 == 2) swap(b[y1], b[y2]);
				c[bl[y1]] += (a[y1] != b[y1]);
				if (y1 != y2) c[bl[y2]] += (a[y2] != b[y2]);
			}
			if (op == 3) {

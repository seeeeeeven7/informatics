...
int main() {
	...
	while (tasks --) {
		...
		sum = sum * 2;
		for (auto p : a) sum /= p.fi;
		// cout << sum << endl;
		if (sum == 0) 
			cout << a[0].se << ' ' << a[a.size() - 1].se << endl;
		else {
			ll ans = 0, ansl = 1, ansr = 1;
			for (ll l = 0; l < (int)a.size(); l++) {
				ll pro = a[l].fi;
				ll sum = a[l].fi;
				for (ll r = l + 1; r < (int)a.size(); r++) {
					pro *= a[r].fi;
					sum += a[r].fi + (a[r].se - a[r - 1].se - 1);
					// cout << l << ' ' << r << ' ' << pro << ' ' << sum << endl;
					if (pro - sum > ans) {
						ans = pro - sum;
						ansl = a[l].se;
						ansr = a[r].se;

...
void dfs1(vector<vector<int>> &e, vector<int> &c, vector<bool> &mark, int f, int i) {
	if (mark[i]) c[i] = 0;
	else c[i] = - n * 2;
	for (int j : e[i])
		if (j != f) {
...
void dfs2(vector<vector<int>> &e, vector<int> &c, vector<bool> &mark, int f, int i, int top, int &ans) {

	if (mark[i]) top = max(top, 0);

	// cout << f << ' ' << i << ' ' << top << ' ' << c[i] << endl;
	if (max(top, c[i]) >= 0) {
		if (ans == -1 || max(top, c[i]) <= ans)
			ans = max(top, c[i]);
	}


	vector<int> cs;
	for (int j : e[i]) if (j != f) cs.pb(j);

	vector<int> lc(1, -n * 2);
	for (int j = 0; j < (int)cs.size(); j++) lc.pb(max(lc[lc.size() - 1], c[cs[j]]));
	vector<int> rc(1, -n * 2);
	for (int j = cs.size() - 1; j >= 0; j--) rc.pb(max(rc[rc.size() - 1], c[cs[j]]));
	
	for (int j = 0; j < (int)cs.size(); j++)
		dfs2(e, c, mark, i, cs[j], max(top, max(lc[j], rc[cs.size() - 1 - j]) + 1) + 1, ans);
}

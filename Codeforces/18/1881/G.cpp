...
int main() {
	...
	while (tasks --) {
		...
		string s; cin >> s;
		s2.clear(); s3.clear();
		for (int i = 0; i + 1 < n; i++) a[i] = ((int)s[i + 1] - (int)s[i] + 26) % 26;
		for (int i = 0; i + 1 < n; i++) check2(i);
		for (int i = 0; i + 2 < n; i++) check3(i);
		while (m --) {
			int t; cin >> t;
			if (t == 1) {
				int l, r, x; cin >> l >> r >> x;
				x = x % 26; l --; r --;
				if (x > 0) {
					if (l > 0) a[l - 1] = (a[l - 1] + x) % 26;
					if (r + 1 < n) a[r] = (a[r] - x + 26) % 26;
					check2(l - 1);
					check3(l - 1);
					check3(l - 2);
					check2(r);
				...
				int l, r; cin >> l >> r; l --; r --;
				auto p2 = s2.lower_bound(l);
				auto p3 = s3.lower_bound(l);
				bool ans = true;
				if (p2 != s2.end()) {
					if ((*p2) + 1 <= r) ans = false;
				}

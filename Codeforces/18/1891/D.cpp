...
int calc(vector<int> &a, vector<int> &b, int n) {
	...
	int i = 0;
	while (n > b[i]) {
		ans = (ans + (b[i] % mo) * (a[i] % mo)) % mo;
		n -= b[i];
		i ++;
	}
	ans = (ans + (n % mo) * (a[i] % mo)) % mo;
	return ans;
}
...
int32_t main() {
	...
	for (int f = 2; f <= 60; f++) {
		int x1 = 1LL << f;
		int x2 = (1LL << (f + 1)) - 1;
		int g1 = acc_sqrt(x1, f);
		int g2 = acc_sqrt(x2, f);
		if (g1 == g2) {
			a.pb(g1);
			b.pb(x2 - x1 + 1);
		}
	...
	while (tasks --) {
		int l, r; cin >> l >> r;
		l -= 3; r -= 3;
		cout << (calc(a, b, r) - calc(a, b, l - 1) + mo) % mo << endl;
	}

...
void dfs(vector<int> &ans, vector<int> &arr, vector<vector<pii>> &f, vector<vector<int>> &g, vector<int> &h, int i) {
	// cout << i << ' ' << h[i] << endl;
	for (auto p : f[i]) {
		// cout << ' ' << p.fi << ' ' << p.se << endl;
		add(arr, p.fi, p.se);	
...
int32_t main() {
	...
	while (tasks --) {
		...
		vector<int> ans(len(g));
		vector<int> arr(q + 2);
		dfs(ans, arr, f, g, h, 1);
		for (int i = 1; i < len(g); i++) cout << ans[i] << ' ';
		cout << endl;
	}

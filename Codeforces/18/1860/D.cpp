	...
	while (cin >> s) {
		...
		for (int i = 0; i < n; i++) 
			if (s[i] == '0')
				...
				p1.pb(i);
		if (p0.size() == 0 || p1.size() == 0) cout << 0 << endl;
		else {
			int m = p0.size() * p1.size() / 2 + p1.size() * (p1.size() - 1) / 2;
			// cout << m << endl;
			vector<vector<vector<int>>> f(p1.size() + 1, vector<vector<int>>(p0.size() + 1, vector<int>(m + 1, n)));
			f[0][0][0] = 0;
				...
				for (int j = 0; j <= (int)p0.size(); j++) {
					for (int k = 0; k <= m; k++) {
						if (f[i][j][k] != n) {
							...
							if (k + p0[j] <= m && j < (int)p0.size()) f[i + 1][j + 1][k + p0[j]] = min(f[i + 1][j + 1][k + p0[j]], f[i][j][k] + 1);
							if (k + p1[i] <= m) f[i + 1][j][k + p1[i]] = min(f[i + 1][j][k + p1[i]], f[i][j][k]);
							if (j < (int)p0.size()) f[i][j + 1][k] = min(f[i][j + 1][k], f[i][j][k]);
						}
					}
				}
			}
			int ans = n;
			for (int j = 0; j <= (int)p0.size(); j++) ans = min(ans, f[p1.size()][j][m]);
			cout << ans << endl;
		}
	}
	return 0;

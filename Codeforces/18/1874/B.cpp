...
int main() {
    ...
    while (tasks --) {
    	...
    	int allmin = a[0], allmax = a[0];
    	for (int i = 0; i < n; i++) {
    		allmin = min(allmin, a[i]);
    		allmax = max(allmax, a[i]);
    	}
    	for (int j = 0; j < m; j++) {
    		allmin = min(allmin, b[j]);
    		allmax = max(allmax, b[j]);
    	}
    	// cout << allmin << ' ' << allmax << endl;
    	while (true) {
    		int p = 0, q = 0, tmp;
    		for (int i = 1; i < n; i++) if (a[i] < a[p]) p = i;
			for (int j = 1; j < m; j++) if (b[j] > b[q]) q = j;
			// cout << k << ' ' << p << ' ' << q << endl;
			if (a[p] < b[q]) {
				tmp = a[p]; a[p] = b[q]; b[q] = tmp;	
			}
			k --;
			if (k == 0) break;
			if (a[p] == allmax && b[q] == allmin && k % 2 == 0) break;
    		for (int i = 1; i < n; i++) if (a[i] > a[p]) p = i;
			for (int j = 1; j < m; j++) if (b[j] < b[q]) q = j;
			if (a[p] > b[q]) {
				tmp = a[p]; a[p] = b[q]; b[q] = tmp;	
			...
			k --;
			if (k == 0) break;
			if (a[p] == allmin && b[q] == allmax && k % 2 == 0) break;
    	}
    	long long ans = 0;

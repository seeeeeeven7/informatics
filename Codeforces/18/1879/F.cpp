...
void merge(int am1, int ap1, int am2, int bm1, int bp1, int bm2, int &cm1, int &cp1, int &cm2) {
...
//    freopen(".out", "w", stdout);
	...
	while (tasks --) {
		...
		cout << m << ' ' << bits << endl;
		vector<vector<int>> m1(m * 2 + 1, vector<int>(bits, -1));
		vector<vector<int>> p1(m * 2 + 1, vector<int>(bits, -1));
		vector<vector<int>> m2(m * 2 + 1, vector<int>(bits, -1));
		for (int i = 0; i < n; i++) merge(-1, -1, -1, h[i], i, -1, m1[a[i]][0], p1[a[i]][0], m2[a[i]][0]);

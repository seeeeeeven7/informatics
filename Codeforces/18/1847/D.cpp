...
int main() {
	...
	while (cin >> n >> m >> k) {
		...
		for (int i = 0, j = 0; i < n; i++) {
			while (j < len(a) && a[j].fi == i) {
				if (a[j].se > 0) se.insert(a[j].se);
				else se.erase(-a[j].se);
				j ++;
			}
			if (se.size() > 0)
				b.pb(mp(*se.begin(), i));
		...
		for (int i = 0; i < n; i++)
			if (s[i] == '1')
				n1 ++;
		int ans = 0;
		for (int i = 0; i < min(n1, len(b)); i++)
			if (s[b[i].se] == '0')
				ans ++;
		while (k--) {
			int p; cin >> p; p--;
			// cout << s << ' ' << p << ' ' << s[p] << ' ' << c[p] << ' ' << n1 << endl;
			if (s[p] == '0') {
				s[p] = '1';
				if (c[p] != -1 && c[p] < n1) ans --;
				n1 ++;
				if (n1 <= len(b) && s[b[n1 - 1].se] == '0') ans ++;
			}
			else {
				if (n1 <= len(b) && s[b[n1 - 1].se] == '0') ans --;
				n1 --;

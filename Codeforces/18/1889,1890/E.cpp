...
void calc(vector<int> &g, vector<pii> &a, int k) {
	int n = len(a);
	// cout << len(a) << endl;
	...
	sort(l.begin(), l.end(), [&a] (int i, int j) { 
		if (a[i].fi == a[j].fi) return a[i].se > a[j].se;
		return a[i].fi < a[j].fi; 
	});
	sort(r.begin(), r.end(), [&a] (int i, int j) { 
	...
	for (int i = 0; i <= min(n - 1, k); i++) {
		...
		while (i + c <= min(n - 1, k)) {
			while (bo[r[j]]) j++;
			// cout << "cj " << c << ' ' << j << endl;
			g[i + c] = min(g[i + c], a[r[j]].se - a[l[i]].fi + 1);
			j ++; c ++;
		}
		bo[l[i]] = true;
	}
...
int32_t main() {
	...
	while (tasks --) {
		...
		sort(p.begin(), p.end());
		vector<pii> a;
		vector<vector<int>> f(2, vector<int>(k + 1, n)); 
		f[0][0] = 0;
		int u = 0, v = 1;
		for (int i = 0, j = 0; i < len(p); i++) {
			if (p[i].se > 0) {
				j++; 
				a.pb(mp(l[p[i].se], r[p[i].se]));
			}
			else j--;
			if (i == len(p) - 1 || p[i].fi != p[i + 1].fi) {
				// cout << j << endl;
				if (j == 0) {
					vector<int> g(k + 1, 0);
					calc(g, a, k);
		...
		int ans = n;
		for (int x = 0; x <= k; x++) ans = min(ans, f[u][x]);
		cout << n - ans << endl;
	}

...
int main() {
	...
	while (tasks --) {
		...
		sort(hi.begin(), hi.end(), [&h](int i, int j) { return h[i] < h[j]; });
		// print1d(hi);
		...
		int q; cin >> q;
		vector<int> a(q), b(q), c(q), qi(q);
		for (int i = 0; i < q; i++) {
			cin >> a[i] >> b[i] >> c[i]; a[i] --; b[i] --;
			qi[i] = i;
		}
		sort(qi.begin(), qi.end(), [&a, &c, &h](int i, int j) { return h[a[i]] + c[i] < h[a[j]] + c[j]; });
		...
		for (int i : qi) {
			...
			int x = findF(f, a[i]), y = findF(f, b[i]);
			ans[i] = bo[x] && x == y;

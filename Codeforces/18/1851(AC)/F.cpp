...
int main() {
	...
	while (tasks --) {
		...
		vector<vector<int>> e(1, vector<int>(3, -1));
		for (int i = 0; i < n; i++) {
			cin >> a[i];
			if (i > 0) {
				int t = 0;
				...
				// cout << i << ' ' << e[t][2] << ' ' << a[i] << ' ' << a[e[t][2]] << ' ' << (((1 << m) - 1 - a[i]) ^ a[e[t][2]]) << endl;
				if ((((1 << m) - 1 - a[i]) ^ a[e[t][2]]) > ans) {
					ans = ((1 << m) - 1 - a[i]) ^ a[e[t][2]];
					ans1 = i;
					ans2 = e[t][2];
				}
			}
		...
		cout << ans1 + 1 << ' ' << ans2 + 1 << ' ' << ((1 << m) - 1 - a[ans1]) << endl;
	}

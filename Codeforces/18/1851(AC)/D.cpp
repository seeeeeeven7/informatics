...
int main() {
	...
	while (tasks --) {
		...
		bool valid = true;
		for (int i = 1; i < n; i++) {
			if (a[i] - a[i - 1] < 1) valid = false;
			else if (a[i] - a[i - 1] > n) {
				if (a[i] - a[i - 1] < 2 * n) q.pb(a[i] - a[i - 1]);
				else valid = false;
			}
			else c[a[i] - a[i - 1]] ++;
		}
		...
		for (int i = 1; i <= n; i++) {
			if (c[i] == 0) p.pb(i);
			else if (c[i] == 2) q.pb(i);
			else if (c[i] != 1) valid = false;
		}
		if (valid) {
			if (len(p) == 1 && len(q) == 0) cout << "YES" << endl;
			else if (len(p) == 2 && len(q) == 1 && p[0] + p[1] == q[0]) cout << "YES" << endl;
			else cout << "NO" << endl;
		}
		else cout << "NO" << endl;
	}

...
int32_t main() {
	...
	while (tasks --) {
		string s; cin >> s;
		int n = s.length();
		vector<vector<int>> right(n + 1, vector<int>(10));
		for (int i = 0; i < 10; i++) right[n][i] = n + 1;
		for (int i = n - 1; i >= 0; i--)
			for (int j = 0; j < 10; j++)
				if (s[i] == '0' + j)
					right[i][j] = i + 1;
		...
		int m; cin >> m;
		string l, r; cin >> l >> r;
		vector<vector<int>> f(m + 1, vector<int>(10, -1));
		for (int j = 0; j < 10; j++)
			f[0][j] = 0;
		bool ans = false;
		for (int i = 0; i <= m; i++)
			for (int j = 0; j < 10; j++) if (f[i][j] != -1) {
				if (f[i][j] == n + 1) ans = true;
				else if (i < m) {
					for (int k = l[i] - '0'; k <= r[i] - '0'; k++)
						if (f[i + 1][k] == -1 || f[i + 1][k] < right[f[i][j]][k])
							f[i + 1][k] = right[f[i][j]][k];
				}
			}
